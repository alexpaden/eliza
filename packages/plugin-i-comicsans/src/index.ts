...
const imageDetection: Action = {
    ...
    handler: async (
        ...
        if (message.content.inReplyTo) {
            ...
            if (parentMessage) {
                ...
                parentImageUrls =
                    parentMessage.content.attachments
                        ?.filter((attachment) =>
                            attachment.contentType?.startsWith("image/")
                        )
                        ?.map((attachment) => attachment.url) || [];
    ...
    examples: [
        [
            {
                user: "{{user1}}",
                content: { text: "Does this message have any images?" },
            },
            {
                user: "{{agentName}}",
                content: {
                    text: "Let me check that message for images",
                    action: "DETECT_IMAGE",
                },
            },
        ],
        [
            {
                user: "{{user1}}",
                content: { text: "Can you scan this for images?" },
            },
            {
                user: "{{agentName}}",
                content: {
                    text: "I'll check if there are any images",
                    action: "DETECT_IMAGE",
                },
            },
        ],
        [
            {
                user: "{{user1}}",
                content: { text: "Check if there's an image here" },
            },
            {
                user: "{{agentName}}",
                content: {
                    text: "I'll look for any images in the message",
                    action: "DETECT_IMAGE",
                },
            },
        ],
...
export const imageDetectionPlugin: Plugin = {
    name: "imageDetection",
    description: "Detects presence of images and Comic Sans in messages",
    actions: [imageDetection],
    evaluators: [],
    providers: [],
};
