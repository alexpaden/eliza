...
export const comicImageDetection: Action = {
    ...
    handler: async (
        ...
        // Check cooldown first before proceeding with expensive operations
        const now = Date.now();
        ...
        if (recentDetection) {
            ...
            callback({ 
                text: "⚠️ Cooldown active: You have already recorded a Comic Sans detection within the last 24 hours. Please try again later.", 
                imageUrls: [] 
            });
            return;
        }

        // Now proceed with profile validation since we're not in cooldown
